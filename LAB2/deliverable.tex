...
\begin{itemize}
\item \textit{pi\_omp\_\_critical.c} where a critical region is used to protect every access to sum, ensuring exclusive access to it
...
Firstly, we have analysed how many synchronisation operations(critical or atomic) are executed in each version.  

POSAR AQUÍ RECOMPTE DE CRITICALS I ATOMICS
...
Next we executed the same programs with the same number of iterations, this time with both 4 and 8 threads, and the execution times when submited are the following (put against the results with 1 thread to compare):

\begin{center}
\begin{tabular}{|| c | c | c | c ||} 
\hline
\textbf{Version} & \textbf{1 thread} & \textbf{4 threads} & \textbf{8 threads} \\ [0.5ex] 
\hline\hline
pi\_omp\_critical & 4.327817s & 36.796537s & 34.572298s \\ 
\hline
pi\_omp\_atomic & 1.796890s & 6.614634s & 7.093226s \\
\hline
pi\_omp\_reduction & 1.832242s & 0.483557s & 0.262972s \\
\hline
pi\_omp\_sumlocal & 1.832944s & 0.480225s & 0.260914s \\ [1ex] 
\hline
pi\_sequential & \multicolumn{3}{| c ||}{1.792886019s}\\
\hline
\end{tabular}
\end{center}
