...
\\
Observing the results on the execution of \textit{pi\_omp\_parallel} with just 1 iteration and a maximum of 24 threads, see in figure \ref{overheadplot1}, we can confirm that the time of the overhead is not constant, it increases linearly when the number of threads increase. Moreover, the overhead per thread tends to decrease due to most of the overhead of the program being created by just parallelizing instead of adding threads. This means that when you parallelize a program a big overhead is created, but then little overhead is added for each thread created. This is why when we add threads, the overhead per thread tends to decrease (up to a point), not much is added but is divided by another threads.
...
With this we conclude our study about overheads with different pragma omp calls.
