...
\item  \textit{pi\_omp\_sumlocal.c}, in this file we have found just a \textit{\#pragma omp critical} outside the loop which merge all the sums into a general sum. This again, as with reduction, will be executed just once for every thread. We hypothesized that we would see a similar overhead to \textit{pi\_omp\_reduction.c} since their implementation is very alike. We used Paraver in order to see their tasks and our hypothesis was confirmed, please see figure \ref{4threadsParaverZoom}. 
\end{itemize}
