...
\\
Observing the results on the execution of \textit{pi\_omp\_parallel} with just 1 iteration and a maximum of 24 threads, see in figure \ref{piqueue}, we can confirm that the time of the overhead is not constant, it increases linearly when the number of threads increase. Moreover, the overhead per thread tends to decrease because when the number of threads increases every thread has to do less calculations.
\\
\medskip

Finally we can confirm that our conclusions are correct since the formulas to compute the overhead are the following:
\begin{itemize}
\item Overhead = $t_1 + num\_threads \times t_2$
\item Overhead per thread $= \frac{t_1+num\_threads\times t_2}{num\_threads}$
\end{itemize}

Being $t_1$ a constant and $t_2$ depends on the number of threads.
